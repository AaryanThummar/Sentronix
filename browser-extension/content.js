...
(function () {
  ...
  chrome.runtime.onMessage.addListener((req, sender, sendResponse) => {
    ...
    } else if (req.action === "SHOW_DOWNLOAD_PROMPT") {
      showDownloadToast(req.downloadId, req.filename);
      sendResponse({ ok: true });
    } else if (req.action === "UPDATE_DOWNLOAD_STATUS") {
      updateDownloadToast(req.downloadId, req.status, req.title, req.message);
      sendResponse({ ok: true });
    }
  });

  function showDownloadToast(downloadId, filename) {
    const existing = document.getElementById("sentronix-download-toast");
    if (existing) existing.remove();

    const toast = document.createElement("div");
    toast.id = "sentronix-download-toast";
    toast.innerHTML = `
      <div class="sentronix-toast-icon">🛡️</div>
      <div class="sentronix-toast-body">
        <div class="sentronix-toast-title">SentroniX Download Defense</div>
        <div class="sentronix-toast-file">File: <strong>${filename}</strong></div>
        <div class="sentronix-toast-desc">Scan this file for malware / steganography before downloading?</div>
        <div class="sentronix-toast-actions">
          <button id="stx-toast-scan" class="stx-btn-scan">🛡️ Scan File</button>
          <button id="stx-toast-asis" class="stx-btn-asis">⬇️ Download As-Is</button>
        </div>
      </div>
      <button id="stx-toast-close" class="stx-toast-close" title="Dismiss">×</button>
    `;

    document.body.appendChild(toast);

    document.getElementById("stx-toast-close").addEventListener("click", () => {
      toast.remove();
    });

    document.getElementById("stx-toast-asis").addEventListener("click", () => {
      chrome.runtime.sendMessage({ type: "RESUME_DOWNLOAD", downloadId: downloadId });
      toast.remove();
    });

    document.getElementById("stx-toast-scan").addEventListener("click", () => {
      const desc = toast.querySelector(".sentronix-toast-desc");
      const actions = toast.querySelector(".sentronix-toast-actions");
      if (desc) desc.innerHTML = "<em>🔍 Analyzing file contents for steganography & malware...</em>";
      if (actions) actions.style.display = "none";

      chrome.runtime.sendMessage({ type: "EXECUTE_DOWNLOAD_SCAN", downloadId: downloadId });
    });
  }

  function updateDownloadToast(downloadId, status, title, message) {
    const toast = document.getElementById("sentronix-download-toast");
    if (!toast) return;

    const iconEl = toast.querySelector(".sentronix-toast-icon");
    const titleEl = toast.querySelector(".sentronix-toast-title");
    const descEl = toast.querySelector(".sentronix-toast-desc");
    const actionsEl = toast.querySelector(".sentronix-toast-actions");
    ...
    if (status === "clean") {
      ...
      if (titleEl) titleEl.innerText = title || "✓ Clean File Verified";
      if (descEl) descEl.innerText = message || "File passed all security checks. Resuming download.";
      ...
      if (titleEl) {
        titleEl.innerText = title || "🚨 Threat Blocked & Quarantined";
        titleEl.style.color = "#991b1b";
      }
      if (descEl) {
        descEl.innerHTML = `<strong style="color:#b91c1c;">${message}</strong>`;
      }
    }
  }
})();
